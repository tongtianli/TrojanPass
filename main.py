...
def main(send_mail: bool = True):
    ...
    net_ids = [net_id.strip() for net_id in os.environ.get('TROJAN_PASS_NETID').split() if net_id]
    net_pws = [net_pw.strip() for net_pw in os.environ.get('TROJAN_PASS_PASSWORD').split() if net_pw]
